...
evaluation = dict(
    interval=4000, metric='mIoU', save_best='mIoU', rule='greater')
...
optimizer = dict(
    ...
    paramwise_cfg=dict(
        custom_keys=dict({
            ...
            dict(lr_mult=25.0, decay_mult=0.0),
            'backbone.sac_scc_beta_dino_logit':
            dict(lr_mult=25.0, decay_mult=0.0),
            'backbone.scc_gate_vlm':
            dict(lr_mult=15.0, decay_mult=0.0),
            'backbone.scc_gate_dino':
            dict(lr_mult=15.0, decay_mult=0.0)
        })))
work_dir = './work_dirs/hpi_eva_vit-l_1e-4_20k-c2m-512'
gpu_ids = range(0, 1)
