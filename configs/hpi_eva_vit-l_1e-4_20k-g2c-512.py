...
runner = dict(type='IterBasedRunner', max_iters=28000)

checkpoint_config = dict(by_epoch=False, interval=12000, max_keep_ckpts=1)
evaluation = dict(interval=12000, metric='mIoU', save_best='mIoU', rule='greater')
...
train_pipeline = [
    ...
    dict(type='Normalize', mean=IMG_MEAN, std=IMG_VAR, to_rgb=True),
...
optimizer = dict(
    ...
    paramwise_cfg=dict(
        custom_keys=dict({
            ...
            dict(lr_mult=1.0, decay_mult=1.0),
            'backbone.sac_scc_beta_vlm_logit': dict(lr_mult=25.0, decay_mult=0.0),
            'backbone.sac_scc_beta_dino_logit': dict(lr_mult=25.0, decay_mult=0.0),
            'backbone.scc_gate_vlm': dict(lr_mult=15.0, decay_mult=0.0),
            'backbone.scc_gate_dino': dict(lr_mult=15.0, decay_mult=0.0),
        })))
...
work_dir = './work_dirs/hpi_eva_vit-l_1e-4_20k-g2c-512'
